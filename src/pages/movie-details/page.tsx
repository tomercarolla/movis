...
export default function Component() {
    ...
    const movie = useMovieStore(state => id ? state.getMovieById(Number(id)) : undefined);

    if (!movie) return <div className="p-4 text-center">Loading...</div>;

    const {title, poster_path: cover, release_date, runtime: duration, genres, vote_average: rating, overview} = movie;
